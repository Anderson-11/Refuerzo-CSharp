...
Console.WriteLine(message + " los impuestos totales son " + mensage2);
Console.WriteLine(ventas1.getInfoCliente("Ander Cisneros"));
Pago ejemplo = new();
...
var Ander = new Persona()
{
    Name = "Ander",
    Age = 20,
};

string json = JsonSerializer.Serialize(Ander);
var myJson = @"{""Name"":""Ander"",""Age"":20}";
Console.WriteLine(json);

Persona? yo = JsonSerializer.Deserialize<Persona>(myJson);
Console.WriteLine(yo?.Name +" "+ yo?.Age);
Console.WriteLine("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
...
var names2 = from alias in nombre
             where  alias.Length > 6
             orderby alias descending 
             select alias;
var nombre3 = nombre.Where(n => n.Length > 7).OrderByDescending(n => n).Select(n => n);
